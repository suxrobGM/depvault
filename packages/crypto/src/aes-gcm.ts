...
/** Encrypt raw bytes with AES-256-GCM, splitting the trailing auth tag into its own field. */
async function encryptBytes(data: ArrayBuffer, dek: CryptoKey): Promise<EncryptedData> {
  const iv = getRandomBytes(IV_LENGTH);
  const result = await crypto.subtle.encrypt({ name: "AES-GCM", iv: asBuffer(iv) }, dek, data);

  const ct = new Uint8Array(result);
  const encryptedData = ct.slice(0, ct.length - AUTH_TAG_LENGTH);
  const authTag = ct.slice(ct.length - AUTH_TAG_LENGTH);
...
/** Decrypt base64-encoded AES-256-GCM fields (ciphertext + separate auth tag) back to bytes. */
...
export async function encrypt(plaintext: string, dek: CryptoKey): Promise<EncryptedData> {
  return encryptBytes(asBuffer(new TextEncoder().encode(plaintext)), dek);
}
...
export async function decrypt(
  ...
  return new TextDecoder().decode(await decryptBytes(ciphertext, iv, authTag, dek));
}
...
export async function decryptBinary(
  ...
  return decryptBytes(ciphertext, iv, authTag, dek);
}
