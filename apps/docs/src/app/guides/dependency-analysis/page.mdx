...
| ------------- | ---------------------------------------------------------------------------- | ------------- |
| `--file`      | Path to a dependency file. When omitted, discovered files are offered as a multi-select. | Auto-detected |
...
In interactive mode, when `--file` is omitted the CLI scans the current directory for dependency files and shows a multi-select prompt (toggle with space, confirm with enter). Every selected file is analyzed in turn.
