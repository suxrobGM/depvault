...
Each config file and secret file is encrypted as a **single whole-file blob** (ciphertext, IV, and auth tag) — DepVault never parses files into individual variables, and the server never sees plaintext. The key hierarchy below is unchanged: one Data Encryption Key per project protects every blob in that project.
