...
DepVault uses end-to-end encryption — your config and secret files are encrypted in your browser before they leave your device. The server stores only ciphertext and cannot read your data.

Config files and secret files are stored the same way — as a **single whole-file blob** (ciphertext, IV, and auth tag) tagged with a kind (config or secret). DepVault never parses files into individual variables, and the server never sees plaintext. The key hierarchy below is unchanged: one Data Encryption Key per project protects every blob in that project.
...
4. Your config and secret files are encrypted with the project's DEK using AES-256-GCM before leaving your browser.
...
- Plaintext config or secret file contents
- Your vault password
- Your recovery key
