...
1. Select a config or secret file (or upload a file) and generate a link.
2. Your browser encrypts the content with a one-time key. The encryption key is embedded in the URL fragment — it never reaches the server.
...
### From a Config or Secret File

1. In the repository view, open a config or secret file and choose **Share**.
