...
Secrets are end-to-end encrypted. When you create a CI token, the project encryption key is securely wrapped with the token. The CLI uses the token to decrypt secrets locally — the server never sees plaintext.
...
Delete tokens at any time from the CI Tokens settings page. Deleted tokens are permanently removed and rejected immediately. Token usage is logged for auditing (timestamp, IP, action, status).
...
| `401 Unauthorized`          | Token is expired, deleted, or invalid.                    |
