...
/// <summary>
/// Downloads each secret file's encrypted blob, decrypts it with the project DEK, and writes the
/// plaintext bytes verbatim to its exact <c>relativePath</c> under the output directory. Binary
/// secrets are restored byte-for-byte.
/// </summary>
public sealed class SecretFilePuller(
    IApiClientFactory clientFactory,
    IOutputFormatter output)
{
    /// <summary>
    /// Pulls the given secret-file entries for a project and writes them to disk. Returns the
    /// number of files successfully written.
    /// </summary>
    public async Task<int> PullAsync(
        string projectId,
        IReadOnlyList<SecretFileEntry> entries,
        string outputDir,
        byte[] dek,
        CancellationToken ct)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            try
            {
                var blob = await client.Api.Projects[projectId].Secrets[entry.Id].Download
                    .GetAsync(cancellationToken: ct);

                if (blob is null || string.IsNullOrEmpty(blob.EncryptedContent))
                {
                    output.PrintError($"Failed to download {entry.RelativePath}: empty response");
                    continue;
                }
            ...
            {
                ApiErrorHandler.HandleError(ex, $"Failed to download {entry.RelativePath}");
            }
        }
